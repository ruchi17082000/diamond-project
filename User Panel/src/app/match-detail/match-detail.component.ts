...
export class MatchDetailComponent implements OnInit, OnDestroy {
  ...
  createRoom() {
    ...
    this.socket.on(
      ...
      function (data: any) {
        // console.warn(data);
        if (data[0]?.length == 0 && data[1]?.length == 0 && data[2]?.length == 0 && data[2][1]?.length == 0 && data[2][2]?.length == 0 && data[2][3]?.length == 0) {
        } else {
          this.matchDetail = data;
          this.matchoddMarket = data[0];
          this.bookmakerMarket = data[1];
          this.sessionMarket = data[2][0];
          this.over_by_over_Market = data[2][1];
          this.ball_by_ballMarket = data[2][2];
          this.meterMarket = data[2][3];
        }
        ...
        // this.sessionMarket=[];
        // this.over_by_over_Market=[];
        // this.ball_by_ballMarket=[];
        // this.meterMarket=[];
